...
def getAbsoluteLink(main: str, found_link: str):
    ...
    if found_link.startswith(main):
        return found_link

    if found_link.startswith(('http', 'ftp', 'tel:', 'mailto:', 'javascript')):
        return None
...
def not_forbidden_content_type(url: str) -> bool:
...
def checkAllUrls(url):
    if not_forbidden_content_type(url):
        parseLinks(url)
    visited_urls.add(url)
